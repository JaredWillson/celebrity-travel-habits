...
d3.json(queryUrl).then(function (data) {
  console.log(data);
  createFeatures(data.____________);


});

function createFeatures(______){   // quakeData is data.features read from the data
  let flightPaths = [];  // initial markers 
  for (i=0;i<flight_data.length;i++){   // iterate each object of the flight_data array
      depLat = ___________[i].dep_latitude;   
      depLon = ___________[i].dep_longitude; 
      arrLat = ___________[i].arr_latitude;
      arrLon = ___________[i].arr_longitude; 
